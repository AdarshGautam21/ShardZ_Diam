...
const BoughtAssets = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      // setAssets(Assets);
      const response = await getBoughtAsset();
            ...
            <div className='bg-gradient-to-br from-cyan-400  via-[#ad9f9f00] to-cyan-400 p-[0.1vw] rounded-[0.5vw]' >
                <div className="bg-[#151414] h-full text-white rounded-[0.5vw] overflow-hidden">
                  <div className=  "w-full relative " >
                    {/* <Link href='/NftPage'> */}
                  <img src={`https://gateway.lighthouse.storage/ipfs/${Buffer.from(asset.data_attr.thumbnailCid, "base64").toString("utf-8")}`}  alt={'thumbnails'} className="w-full h-full p-[1vw]" />
                  {/* </Link> */}
                  </div>
                    <div className="p-[1vw]">
                    {/* <Link href='/NftPage'> */}
                        <p className="  text-[4vw] md:text-[1vw]  font-semibold mb-2">{Buffer.from(asset.data_attr.title, "base64").toString("utf-8")}</p>
                        {/* </Link> */}
                        <div className='flex text-center items-center justify-between text-[2vw] md:text-[0.8vw] text-[#808191] space-x-[0.5vw]' >
                            ...
                            <p className=' text-[3vw] md:text-[1.5vw] font-semibold bg-gradient-to-r inline-block text-transparent bg-clip-text from-[#33C1EE] via-[#8DDCF5]  to-[#FFFFFF] ' >{Number(asset.balance)}/100</p>
                        </div>
